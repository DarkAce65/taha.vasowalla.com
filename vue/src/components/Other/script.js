...
require("@/assets/lib/TrackballControls.js");
import perlin from "@/assets/lib/perlin.js";

var componentData = {r: true};
...
function displaceSatelliteGeometry(satelliteGeometry) {
	...
	for(var i = 0; i < satelliteGeometry.vertices.length; i++) {
		...
		satelliteGeometry.vertices[i].setLength(3 + Math.abs(perlin.noise.simplex3(v.x, v.y, v.z)) * 2);
	}
}
...
componentData.renderer = renderer;
componentData.clock = clock;
componentData.scene = scene;
componentData.camera = camera;
componentData.controls = controls;
componentData.animation = animation;
componentData.planet = planet;
componentData.satellites = satellites;
componentData.resize = resize;
	...
	methods: {
		render: function() {
			if(this.r) {
				...
				this.planet.rotation.z += delta * this.animation.planetRotation / 5;
				for(var i = 0; i < this.satellites.length; i++) {
					this.satellites[i].rotation.z += this.satellites[i].orbitSpeed * delta * this.animation.satelliteOrbit;
				}
			}
		}
	},
